...
def test_service_reply_to_username_with_default_count(client):
    username = "Vasya"
...
def test_service_reply_to_escaped_username(client):
    ...
    expected_greeting_count = 15
    response = client.get(
        f"/hello/{non_escaped_tag}{username}/{expected_greeting_count}"
    )
    response_text = response.data.decode(response.charset)
    petya_count = response_text.count(username)
    assert expected_greeting_count == petya_count
...
def test_hello_username_with_slash(client):
    username = "Vasya"
...
def test_service_reply_to_username_with_too_many_num(client):
    ...
    response = client.get(
        f"/hello/{username}/{supplied_greeting_count}", follow_redirects=True
    )
    response_text = response.data.decode(response.charset)
    petya_count = response_text.count(username)
    assert expected_greeting_count == petya_count
...
def test_service_reply_to_username_with_really_too_many_num(client):
    username = "Petya"
    supplied_greeting_count = REALLY_TOO_MANY_COUNT
    response = client.get(
        f"/hello/{username}/{supplied_greeting_count}", follow_redirects=True
    )
    ...
    response_text = response.data.decode(response.charset)
    petya_count = response_text.count(username)
    assert 0 == response_text.count(username)
